...
- **Acquisition + free trial (decided 2026-07-26)**: the free first issue IS the trial — card required at signup, subscription auto-starts at ship + 7 days, instant no-questions refunds (№ 1 arrived in 7 days, so some charges land right at arrival). US-only at launch, trial and paid — Canada waits for confirmed tax/customs economics. The trial is earned, not timed: nothing prints until the queue fills, so dormant trials cost ~$0 and linger free forever (activation measured as shipped-within-90-days); one trial per Stripe card fingerprint. **Email philosophy exception**: the trial-ship notice is the one legally required email (FTC negative-option rule + card-network physical-goods trial rules) — dry and direct, sent at ship — and full surprise resumes from issue № 2 onward. Annual plan $490/yr (2 months free) offered at conversion; under the pessimistic churn prior an annual taker's contribution (~$311) beats a monthly subscriber's entire expected LTV (~$279). Planning priors: ship→paid c = 50% (stress 25%), churn 12%/mo → CAC ceiling ~$93, trial-only CAC $27–54, self-funding above c = 40%. Channel order: organic → referral gift issues → newsletter sponsorships (gated on observed c ≥ ~37%); paid social off the table under the prior. Full model in `ECONOMICS.md` § Acquisition model. **Built same day**: open-ended Checkout trial (730-day hold — Stripe can't say "until the queue fills", so the closer pins trial_end to ship+7d at first SHIPPED via a new closer→api service binding + `POST /trial/shipped`, daily-sweep backstop mirroring owed-trees), the one notice email, `/billing` (portal bounce, the cancel link), `/annual` (price switch, no proration), card-fingerprint dedupe (repeat → admin alert, not auto-block), trial columns in migration 0016. Checkout shipping is now US-only.
